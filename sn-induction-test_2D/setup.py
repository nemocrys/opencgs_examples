...
def geometry(config, sim_dir="./simdata/_test", name="cz_induction", visualize=False):
    ...
    for shape in [
        # melt, crystal, seed, ax_top,
        crucible, ins, adp, ax_bt, vessel]:
...
def simulation(model, config, sim_dir="./simdata/_test", mat_config={}):
    ...
    # solver for heatfluxes
    solver_bndsum = elmer.load_solver("boundary-scalars", sim.sim, "./config_elmer.yml")
    sim._eqn_main.solvers.append(solver_bndsum)
    # forces
    ...
    # heat flux with SaveScalars
    for bnd in ["bnd_vessel_outside", "bnd_inductor_inside", "if_axbt_vessel"]:
        sim[bnd].data.update({"save scalars": True})
    ...
    sim.heat_flux_computation(sim["inductor"], sim["bnd_inductor_outside"])
    sim.heat_flux_computation(sim["inductor"], sim["bnd_inductor_inside"])
