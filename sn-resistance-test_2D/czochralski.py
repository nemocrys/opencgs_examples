...
def melt(
    ...
    if crystal_radius == 0:  # no meniscus, no phase interface
        ...
        if rho == 0:
            raise ValueError("density zero in cz.melt compute meniscus")
    
        meniscus_x, meniscus_y, _, _, _ = menicus_hurle(crystal_radius, 0, beta, gamma, rho, res=res)
        # meniscus_x, meniscus_y, _ = meniscus_landau(crystal_radius, beta, gamma, rho)
