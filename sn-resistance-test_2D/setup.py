...
def geometry(config, sim_dir="./simdata/_test", name="cz_resistance", visualize=False, mat_config_file="config_mat.yml"):
    ...
    ins_crc = cz.crucible_support(
        model, 2, **config["crucible_insulation"], top_shape=crucible, name="crucible_insulation"
    )
    adp = cz.crucible_adapter(model, 2, **config["crucible_adapter"], top_shape=ins_crc)
    ...
    heater = cz.heater(model, 2, **config["heater"], vessel=vessel)
    if config["insulation"]["bottom_t"] != 0:
        insulaton = cz.insulation(model, 2, **config["insulation"], vessel=vessel)
    ...
    for shape in [crucible, ins_crc, adp, ax_bt, vessel]:
...
def simulation(model, config, sim_dir="./simdata/_test", mat_config={}):
    ...
    # heat flux computation -- always body AND boundary needded
        # TODO doesn't work with 2D T-dependent heat conductivity of insulation
...
if __name__ == "__main__":
    ...
    model = geometry(geo_config, visualize=False)
